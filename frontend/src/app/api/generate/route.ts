...
export async function POST(request: Request) {
    try {
        ...
        if (!response.ok) {
            ...
            console.error(`n8n /generate error (${response.status}):`, text);
        ...
        return NextResponse.json({ error: "Failed to communicate with automation engine", message: error.message }, { status: 500 });
    }
}
