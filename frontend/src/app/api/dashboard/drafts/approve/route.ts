...
export async function POST(request: Request) {
    try {
        ...
        // Trigger secondary n8n webhook for social workflow
        const webhookUrl = process.env.N8N_PART_B_WEBHOOK_URL;
        if (webhookUrl) {
            try {
                // Fire and forget, or wait for response depending on strictness. 
                // Awaiting ensures we don't return success until it's fired.
                ...
                // We're returning success anyway since the DB update succeeded
            }
        }
